...
const artiIdx: Record<string, Panel.ArtIdx> = {
	EQUIP_BRACER: "arti1",
...
export class EnKa {
	private readonly artifact: Panel.EnKaArtifact;

	constructor() {
		const formatArtifact: Panel.EnKaArtifact = {};
		const artifact = metaManagement.getMeta( "enka/artifact" );
	...
	/* 获取用户数据 */
	public getDetailInfo( data: Panel.EnKa ): Panel.Detail {
		if ( !data.avatarInfoList ) {
			throw new Error( "当前用户展示角色为空" );
		}
		const avatars: Panel.Avatar[] = data.avatarInfoList.map( chara => {
			const avatarId = chara["avatarId"];
			const level: string | undefined = chara.propMap['4001'].val;
			const avatarInfo = metaManagement.getMeta( "meta/character" )[ avatarId ];
			return {
				id: avatarId,
				name: avatarInfo ? avatarInfo.name : "",
	...
	private getOverviewAttr( data: Record<string, number> ): Panel.Overview[] {
		const attrList: Panel.Overview[] = [];
		for ( const attr of attrOverviewMap ) {
			...
			const metaData = metaManagement.getMeta( "meta/attr_icon" );
			attrList.push( {
				attr: attr.label,
				icon: metaData[attr.label]?.icon,
				color: metaData[attr.label]?.color || "",
	...
	/* 获取武器信息 */
	private getWeapon( data: Array<Panel.EnKaEquip> ): Panel.Weapon {
		const weaponData = data.find( d => d.flat.itemType === "ITEM_WEAPON" );
		if ( !weaponData ) {
			throw new Error( "enka获取武器数据异常" );
		}
		const { weapon, flat } = <Panel.EnKaWeaponEquip>weaponData;
		const attrs: Panel.ArtAttr[] = flat.weaponStats.map( s => {
			return this.getArtInfo( s );
		} );
		const affixValues = weapon.affixMap ? Object.values( weapon.affixMap ) : [ 0 ];

		return {
			name: metaManagement.getMeta( "enka/meta" )[flat.nameTextMapHash] || "",
			star: flat.rankLevel,
			level: weapon.level,
	...
	/* 获取圣遗物信息 */
	private getArtifact( data: Array<Panel.EnKaEquip> ): { list: Array<Panel.Artifact | {}>, effects: Array<Panel.Effect> } {
		/* 圣遗物属性对象 */
		const ret: Array<Panel.Artifact | {}> = new Array( 5 ).fill( {} );

		/* 统计圣遗物套装 */
		const tmpSetBucket: Record<string, any> = {};

		/* 过滤武器对象 */
		const reliquaryData = <Panel.EnKaReliquaryEquip[]>data.filter( d => d.flat.itemType !== "ITEM_WEAPON" );
		...
		for ( const { flat, reliquary } of reliquaryData ) {
			...
			const artShirtName = metaManagement.getMeta( "enka/meta" )[flat.setNameTextMapHash] || "";
	...
	/* 获取天赋信息 */
	private getSkill( charId: number, skillLevelMap: Record<string, number>, extSkillLevelMap: Record<string, number> = {} ): Panel.Skill {
		let { Skills, ProudMap } = metaManagement.getMeta( "enka/chara" )[charId] || {};
		if ( !Skills || !ProudMap ) {
			return {};
		}
		const charaName = metaManagement.getMeta( "meta/character" )[charId]?.name || "";
		/* 是否存在第四个技能（例如神里冲刺） */
		const exSkillIconPath = bot.file.getFilePath( `genshin/adachi-assets/character/${ charaName }/icon/skill_3.webp`, "plugin" );
		const hasExSkill = bot.file.isExistSync( exSkillIconPath );
		/* 映射对象，a 普攻 s 战技 e 爆发 */
		const idxMap = { a: "0", s: "1", e: hasExSkill ? "3" : "2" };
	...
	/* 获取圣遗物属性信息，[属性名, 值] */
	private getArtInfo( data: { mainPropId?: string; appendPropId?: string; statValue: number } ): Panel.ArtAttr {
		...
		return {
			...
			icon: metaData[attrName]?.icon,
			color: metaData[attrName]?.color || "",
		}
	}
}